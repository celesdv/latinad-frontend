    ...
    return (
        <div className="flex flex-col items-center justify-center p-6 h-screen">
            <form
                ...
                <h3 className="text-xl text-neutral-100 text-center font-bold">Iniciar Sesión</h3>
                <label
                    className="relative rounded-lg borde"
                >
                    <span
                        className="absolute left-2 -translate-y-1/2 pointer-events-none rounded px-1 bg-sky-600 text-neutral-100"
                    >
                        Email
                    </span>
                    <input
                ...
                <div>
                    {errors.email &&
                        (<p className="text-sm text-center text-red-200">Debe ingresar un correo electrónico válido</p>)}
                    {errors.password &&
                        (<p className="text-sm text-center text-red-200">Debe ingresar una clave para válida</p>)}
                    {loginStatus === LOGIN_STATUS.ERROR &&
                        (<p className="text-sm text-center text-red-200">Correo electrónico o clave incorrectos</p>)}
                </div>

                {loginStatus != LOGIN_STATUS.UPLOADING &&
                    (<button type="submit" className="flex bg-neutral-100 text-sky-800 font-bold uppercase rounded py-3 justify-center">Ingresar</button>)
                }
                {loginStatus === LOGIN_STATUS.UPLOADING &&
                    (<Loader />)
                }
